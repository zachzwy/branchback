...
function loadConfig(): OllamaConfig {
  ...
  const tempRaw =
    process.env.OLLAMA_BRIEF_TEMPERATURE ?? process.env.OLLAMA_TEMPERATURE;
  return {
    baseUrl: process.env.OLLAMA_BASE_URL ?? "http://127.0.0.1:11434",
    model:
      process.env.OLLAMA_BRIEF_MODEL ??
...
function buildProvider(): SynthProvider {
  ...
  return {
    ...
    moduleBase: "brief_synthesizer_ollama",
    call: (prompt) => callOllama(cfg, prompt),
  };
}

export function createOllamaBriefSynthesizer(): BriefSynthesizer {
